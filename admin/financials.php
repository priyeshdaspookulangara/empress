...
// Split 50:50 of company inflow/retains: 20% Charity, 30% Company Retained
...
<div class="space-y-8">
    <div class="glass-card p-6 rounded-3xl border border-gold/30 flex justify-between items-center">
        <div>
            <h1 class="text-2xl font-extrabold gold-gradient-text">Financial Audit & Reserve Ledger</h1>
            <p class="text-xs text-champagne/70 mt-1">Master breakdown of 60:40 wallet distributions, 50% Company Retains (20% Charity / 30% Operational)</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div class="glass-card p-6 rounded-3xl border border-gold/40">
            <span class="text-xs font-bold text-gold uppercase tracking-wider">Total Commission Distributed</span>
            <div class="text-3xl font-extrabold gold-gradient-text mt-2">₹<?php echo number_format($wTotals['total_earnings'], 2); ?></div>
            <p class="text-[11px] text-champagne/50 mt-1">Sum of all level payouts credited</p>
        </div>
        ...
        <div class="glass-card p-6 rounded-3xl border border-emerald-500/40 bg-emerald-500/5">
            ...
            <div class="text-3xl font-extrabold text-emerald-400 mt-2">₹<?php echo number_format($wTotals['total_user_wallet'], 2); ?></div>
            <p class="text-[11px] text-champagne/50 mt-1">Eligible for member withdrawals</p>
        </div>
        ...
        <div class="glass-card p-6 rounded-3xl border border-blue-500/40 bg-blue-500/5">
            ...
            <div class="text-3xl font-extrabold text-blue-400 mt-2">₹<?php echo number_format($charityShare, 2); ?></div>
            <p class="text-[11px] text-champagne/50 mt-1">20 out of 50 company allocation</p>
        </div>
        ...
        <div class="glass-card p-6 rounded-3xl border border-amber-500/40 bg-amber-500/5">
            ...
            <div class="text-3xl font-extrabold text-amber-400 mt-2">₹<?php echo number_format($companyRetainedShare, 2); ?></div>
            <p class="text-[11px] text-champagne/50 mt-1">30 out of 50 company allocation</p>
        </div>
    </div>

    <!-- Approved Payout Metrics Card -->
    <div class="glass-card p-6 rounded-3xl border border-gold/20 flex flex-col md:flex-row justify-between items-center gap-4">
        <div>
            <h3 class="text-lg font-bold text-gold">Total Approved Bank Transfers</h3>
            <p class="text-xs text-champagne/70 mt-0.5">Disbursed to members after mandatory KYC approval</p>
        </div>
        <div class="text-2xl font-extrabold text-emerald-400 font-mono">
            ₹<?php echo number_format($totalApprovedPayouts, 2); ?>
        </div>
    </div>
            ...
            <table class="w-full text-left border-collapse text-xs">
                ...
                <tbody class="divide-y divide-gold/10 text-champagne">
                    <?php if (empty($allTransactions)): ?>
                        <tr>
                            <td colspan="8" class="p-4 text-center text-champagne/50">No transactions recorded in system.</td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($allTransactions as $tx): ?>
                            <tr>
                                <td class="p-3 font-mono text-gold/80">#TX-<?php echo $tx['id']; ?></td>
                                <td class="p-3">
                                    <div class="font-bold text-gold font-mono"><?php echo htmlspecialchars($tx['member_id']); ?></div>
                                    <div class="text-champagne/70 text-[11px]"><?php echo htmlspecialchars($tx['name'] ?? ''); ?></div>
                                </td>
                                ...
                                <td class="p-3 text-champagne/80 max-w-xs"><?php echo htmlspecialchars($tx['description']); ?></td>
