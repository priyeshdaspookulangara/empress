...
function parseMemberInsertSQL($sql) {
    ...
    if (!empty($tuples[1])) {
        foreach ($tuples[1] as $rawTuple) {
            // Parse CSV values accounting for quotes
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($actionType === 'reset_database') {
        ...
        if (empty($parsed)) {
            // Fallback: line by line or raw text
            $lines = explode("\n", $sqlInput);
...
<div class="space-y-6">
    <div class="glass-card p-6 rounded-3xl border border-gold/30 flex justify-between items-center">
        <div>
            <h1 class="text-2xl font-extrabold gold-gradient-text">SQL Member Import & Direct Registration Engine</h1>
            <p class="text-xs text-champagne/70 mt-1">Paste SQL INSERT statements or bulk execute direct registrations without ePINs. Triggers global BFS spillover, wallet setup, and 50:50 payouts automatically.</p>
        </div>
    </div>
    ...
    <?php if (!empty($results)): ?>
        <div class="glass-card p-6 rounded-3xl border border-emerald-500/40 space-y-4">
            <div class="flex justify-between items-center border-b border-gold/20 pb-3">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse text-xs">
                    ...
                    <tbody class="divide-y divide-gold/10 text-champagne font-mono text-[11px]">
                        <?php foreach ($results as $res): ?>
                            <tr>
                                <td class="p-2.5 font-bold text-gold"><?php echo htmlspecialchars($res['member_id'] ?? 'N/A'); ?></td>
                                ...
                                <td class="p-2.5 text-champagne/70"><?php echo htmlspecialchars($res['sponsor_id'] ?? 'ROOT'); ?></td>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <!-- SQL Import Form -->
        <div class="lg:col-span-2 glass-card p-6 rounded-3xl border border-gold/20 space-y-4">
            ...
            <p class="text-xs text-champagne/70">
            ...
            <form method="POST" class="space-y-4">
                ...
                <div>
                    <label class="block text-xs font-semibold text-gold mb-1">SQL Query Input</label>
...
('Burfee Cart 2', 'member2@example.com', '9876543211', 'EMP100000');" class="w-full bg-obsidian/80 border border-gold/30 rounded-xl p-3 text-xs text-champagne font-mono focus:outline-none focus:border-gold"></textarea>
                </div>

                <div class="flex justify-between items-center pt-2">
                    <span class="text-[11px] text-champagne/50">Supports bulk INSERT statements with multiple tuple values.</span>
                    <button type="submit" class="gold-button px-6 py-2.5 rounded-xl text-xs font-bold flex items-center gap-2">
        ...
        <div class="space-y-6">
            ...
            <form method="POST" class="space-y-4 text-xs">
                ...
                <div>
                    ...
                    <input type="text" name="bulk_sponsor_id" value="EMP100000" class="w-full bg-obsidian/80 border border-gold/30 rounded-xl px-3 py-2 text-champagne focus:outline-none focus:border-gold font-mono">
                </div>

                <div>
                    <label class="block font-semibold text-gold mb-1">Member Name Prefix</label>
                    <input type="text" name="bulk_name" value="Burfee Cart" class="w-full bg-obsidian/80 border border-gold/30 rounded-xl px-3 py-2 text-champagne focus:outline-none focus:border-gold">
                </div>

                <div>
                    <label class="block font-semibold text-gold mb-1">Number of Members to Create</label>
                    <input type="number" name="single_count" value="3" min="1" max="100" class="w-full bg-obsidian/80 border border-gold/30 rounded-xl px-3 py-2 text-champagne focus:outline-none focus:border-gold font-mono">
                </div>
