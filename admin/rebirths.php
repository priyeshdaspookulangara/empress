...
<div class="space-y-6">
    <div class="glass-card p-6 rounded-3xl border border-neon-cyan/30 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <!-- Search Bar -->
        <form action="/admin/rebirths.php" method="GET" class="flex gap-2 w-full md:w-auto">
            ...
            <?php if (!empty($search)): ?>
                <a href="/admin/rebirths.php" class="glass-card border border-neon-cyan/30 hover:bg-neon-cyan/10 text-neon-cyan px-3 py-2 rounded-xl text-xs flex items-center">Reset</a>
            <?php endif; ?>
        </form>
    </div>
                    ...
                    <?php else: ?>
                        <?php foreach ($rebirthNodes as $rn): ?>
                            <tr>
                                ...
                                <td class="p-3">
                                    <a href="/admin/matrix_tree.php?member_id=<?php echo $rn['member_id']; ?>" class="neon-button px-2.5 py-1 rounded text-[10px] font-bold inline-flex items-center gap-1">
