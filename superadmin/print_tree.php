    ...
    <div class="glass-card p-4 md:p-6 rounded-2xl border border-neon-cyan/30 overflow-x-auto custom-scrollbar bg-obsidian/90">
        ...
        <?php else: ?>
            <div class="min-w-[1400px] mx-auto text-center space-y-6">
                ...
                <div>
                    <div class="text-[10px] font-bold uppercase tracking-widest text-neon-cyan mb-1 no-print">Root Node (Level 0)</div>
                    <div class="inline-block">
                        <div class="node-card px-4 py-2 rounded-xl bg-neon-cyan/20 border-2 border-neon-cyan shadow-lg shadow-neon-cyan/20 text-center min-w-[180px]">
                            <div class="text-xs font-black text-ice node-name truncate max-w-[200px] mx-auto"><?php echo htmlspecialchars($treeData['name']); ?></div>
                            <div class="text-[10px] font-mono text-neon-cyan font-bold node-id"><?php echo htmlspecialchars($treeData['member_id']); ?></div>
                        </div>
                    </div>
                </div>
                ...
                <!-- LEVEL 1 (3 Slots) -->
                <div class="space-y-1">
                    <div class="text-[10px] font-bold uppercase tracking-widest text-neon-cyan/80 no-print">Level 1 (3 Nodes)</div>
                    <div class="grid grid-cols-3 gap-2">
                        <?php for ($p1 = 1; $p1 <= 3; $p1++): ?>
                            <?php $nodeL1 = $treeData['children'][$p1] ?? null; ?>
                            <div class="p-1 rounded-xl border border-neon-cyan/20 bg-obsidian/50">
                                <?php if ($nodeL1): ?>
                                    <div class="node-card p-2 rounded-lg bg-neon-cyan/10 border border-neon-cyan/40 text-center mb-2">
                                        <div class="text-[11px] font-bold text-ice node-name truncate"><?php echo htmlspecialchars($nodeL1['name']); ?></div>
                                        <div class="text-[9px] font-mono text-neon-cyan node-id"><?php echo htmlspecialchars($nodeL1['member_id']); ?></div>
                                    </div>
                                <?php else: ?>
                                    <div class="node-card empty-node p-2 rounded-lg border border-dashed border-neon-cyan/20 text-center text-[10px] text-neon-cyan/40 mb-2">
                                ...
                                <div class="grid grid-cols-3 gap-1">
                                    <?php for ($p2 = 1; $p2 <= 3; $p2++): ?>
                                        ...
                                        <div class="p-0.5 rounded-lg border border-neon-cyan/10 bg-black/20">
                                            <?php if ($nodeL2): ?>
                                                <div class="node-card p-1.5 rounded bg-neon-cyan/10 border border-neon-cyan/30 text-center mb-1">
                                                    <div class="text-[10px] font-bold text-ice node-name truncate" title="<?php echo htmlspecialchars($nodeL2['name']); ?>"><?php echo htmlspecialchars($nodeL2['name']); ?></div>
                                                    <div class="text-[8px] font-mono text-neon-cyan node-id"><?php echo htmlspecialchars($nodeL2['member_id']); ?></div>
                                                </div>
                                            <?php else: ?>
                                                <div class="node-card empty-node p-1 rounded border border-dashed border-neon-cyan/10 text-center text-[8px] text-neon-cyan/30 mb-1">
                                                    Open
                                                </div>
                                            <?php endif; ?>
                                            ...
                                            <div class="grid grid-cols-3 gap-0.5">
                                                <?php for ($p3 = 1; $p3 <= 3; $p3++): ?>
                                                    ...
                                                    <div class="p-0.5 rounded border border-neon-cyan/10 bg-black/30">
                                                        <?php if ($nodeL3): ?>
                                                            <div class="node-card p-1 rounded bg-neon-cyan/5 border border-neon-cyan/20 text-center mb-0.5">
                                                                <div class="text-[8px] font-bold text-ice node-name truncate" title="<?php echo htmlspecialchars($nodeL3['name']); ?>"><?php echo htmlspecialchars($nodeL3['name']); ?></div>
                                                                <div class="text-[7px] font-mono text-neon-cyan/80 node-id"><?php echo htmlspecialchars($nodeL3['member_id']); ?></div>
                                                            </div>
                                                        <?php else: ?>
                                                            <div class="node-card empty-node p-0.5 rounded border border-dashed border-neon-cyan/10 text-center text-[7px] text-neon-cyan/20 mb-0.5">
                                                                -
                                                            </div>
                                                        <?php endif; ?>
                                                            ...
                                                            <?php for ($p4 = 1; $p4 <= 3; $p4++): ?>
                                                                ...
                                                                <?php if ($nodeL4): ?>
                                                                    <div class="node-card p-0.5 rounded bg-emerald-950/40 border border-emerald-500/30 text-center" title="<?php echo htmlspecialchars($nodeL4['name']) . ' (' . htmlspecialchars($nodeL4['member_id']) . ')'; ?>">
                                                                        ...
                                                                        <div class="text-[6px] font-mono text-neon-cyan/90 node-id truncate"><?php echo htmlspecialchars($nodeL4['member_id']); ?></div>
                                                                    </div>
                                                                <?php else: ?>
                                                                    <div class="node-card empty-node p-0.5 rounded border border-dashed border-neon-cyan/5 text-center text-[6px] text-neon-cyan/20">
                                                                        .
                                                                    </div>
                                                                <?php endif; ?>
                                                            <?php endfor; ?>
