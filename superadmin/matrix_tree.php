...
<div class="space-y-6">
    <div class="glass-card p-6 rounded-3xl border border-gold/30 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div class="flex flex-wrap gap-2 items-center">
            <a href="/superadmin/print_tree.php?member_id=<?php echo urlencode($targetId); ?>" class="bg-emerald-600 hover:bg-emerald-500 text-white font-bold px-3.5 py-2 rounded-xl text-xs flex items-center gap-1.5 shadow-lg transition">
                <i class="fa-solid fa-print"></i> 4-Level Print View
            </a>
            <form action="/superadmin/matrix_tree.php" method="GET" class="flex gap-2">
                <input type="text" name="member_id" value="<?php echo htmlspecialchars($targetId); ?>" placeholder="EMP100001" class="bg-obsidian/80 border border-neon-cyan/30 rounded-xl px-4 py-2 text-xs text-ice uppercase font-mono focus:outline-none focus:border-neon-cyan">
                <button type="submit" class="bg-neon-cyan/20 border border-neon-cyan text-neon-cyan hover:bg-neon-cyan/30 px-4 py-2 rounded-xl text-xs font-bold">Inspect Tree</button>
                <?php if ($targetId !== 'EMP100000'): ?>
                    <a href="/superadmin/matrix_tree.php?member_id=EMP100000" class="glass-card border border-neon-cyan/30 hover:bg-neon-cyan/10 text-neon-cyan px-3 py-2 rounded-xl text-xs flex items-center">Reset Root</a>
                <?php endif; ?>
            </form>
        </div>
    </div>
                ...
                <div class="grid grid-cols-3 gap-6">
                    <?php for ($pos = 1; $pos <= 3; $pos++): ?>
                        ...
                        <div class="flex flex-col items-center">
                            <?php if ($child): ?>
                                <a href="/superadmin/matrix_tree.php?member_id=<?php echo $child['member_id']; ?>" class="w-24 h-24 rounded-xl bg-obsidian border border-gold/50 flex flex-col items-center justify-center p-2 hover:border-gold hover:bg-gold/10 transition group">
                                ...
                                <div class="grid grid-cols-3 gap-1 w-full">
                                    <?php for ($subPos = 1; $subPos <= 3; $subPos++): ?>
                                        ...
                                        <div class="text-center">
                                            <?php if ($subChild): ?>
                                                <a href="/superadmin/matrix_tree.php?member_id=<?php echo $subChild['member_id']; ?>" class="w-12 h-12 rounded-lg bg-gold/10 border border-gold/40 mx-auto flex flex-col items-center justify-center p-0.5 hover:border-gold transition block" title="Drill into <?php echo htmlspecialchars($subChild['member_id']); ?>">
